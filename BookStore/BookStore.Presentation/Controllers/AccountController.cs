...
namespace BookStore.Presentation.Controllers
{
    public class AccountController : Controller
    {
        ...
        {
            if (User.Identity.IsAuthenticated && User.Identity.IsRole("Administrators"))
            {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (photo != null && photo.ContentLength > 0)
                {
                    try
                    {
                        ...
                        string path = Path.Combine(Server.MapPath(ImagesPath), Path.GetFileName(photo.FileName));
                        photo.SaveAs(path);
                    }
                    catch (Exception)
                    {
